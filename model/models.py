...
class MultiLayerPerceptronModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.mlp(torch.cat(embed_x, 1))
...
class DeepFieldWeightedFactorizationMachineModel(torch.nn.Module):
    ...
    def __init__(self, field_dims, embed_dim, mlp_dims, dropout=0.0, use_lw=False, use_fwlw=False, use_emb_bag=False, use_qr_emb=False, qr_collisions=4, quantize_dnn=False, batch_norm=True):
        super().__init__()
